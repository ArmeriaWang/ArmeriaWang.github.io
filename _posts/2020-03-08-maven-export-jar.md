...
完成`P1.jar`、`P2.jar`和`P3.jar`的打包之后，别忘了将`pom.xml`中的`<mainClass>`和`<outputFile>`改回原先的值。今后，无论怎么修改代码，都只需要用Maven的一键package流即可完成编译和打包。

对于Lab2、Lab3等之后的实验，也只需将所有Lab1改成LabX，并修改`Root`入口类中各`main`方法所属的类名做对应的更改即可，几乎不用做任何大的改动，非常方便。
