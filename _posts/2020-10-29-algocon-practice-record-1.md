...
因此点分治求出重心$c$之后，子树中的答案递归计算；现在集中精力求经过$c$的路径数（即点对数）。我们需要维护两个东西：$s_u$和$m_u$分别表示$c$到$u$路径上的点权和以及最大点权。枚举路径的一端$u$，则需要统计的路径数就是满足$s_{u}+s_{v}-a_{c}>2 \max \left\{m_{u}, m_{v}\right\}$的结点$v$的数量。
